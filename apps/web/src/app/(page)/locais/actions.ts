...
import { alterarStatusLocal } from '@/http/alterar-status-local'
import { criarLocal } from '@/http/criar-local'
import { editarLocal } from '@/http/editar-local'
...
const alterarStatusLocalSchema = z.object({
  id: z.string(),
})

const alterarStatusLocalAction = async (formData: FormData) => {
  const result = alterarStatusLocalSchema.safeParse(
...
const editarLocalAction = async (formData: FormData) => {
  const result = editarLocalSchema.safeParse(Object.fromEntries(formData))
  ...
  const { id, nome, avatarUrl } = result.data

  try {
    await editarLocal({ id, nome, avatarUrl })
...
export { alterarStatusLocalAction, criarLocalAction, editarLocalAction }
